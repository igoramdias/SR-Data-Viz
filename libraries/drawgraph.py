...
def text_contour(txt, isTitle=None, color=None):
    txt = txt.split(' ', 1)[0]
    txt = txt.upper()
    if (TEMPLATE != 'SBT_LOWER') or (isTitle):
        return f'<span style="fill-opacity: 0; stroke: {color}; stroke-opacity: 1; clip-path: inset(2.9rem 0rem 2.6rem 0rem)">{txt}</span>'
    else:
        return f'<span style="fill-opacity: 0; stroke: {color}; stroke-opacity: 1;">{txt}</span>'
...
def index_draw_graph(fig, annot, symbol, n_months, val_col=None):
    ...
    for pos in range(size):
        ...
        if graph_type[:3] == 'bar' or (graph_type == 'step' and is_last):
            ...
            elif TEMPLATE == 'SBT_LOWER' and is_last:
                txt_sep = txt.split(',')
                if '-' not in txt_sep[0]:
                    color = '#50B7F8'
                    dist_x_fst = 150
                    dist_x_scd = dist_x_fst + 135
                    dist_x_trd = dist_x_fst + 300
                else:
                    color = '#FF657F'
                    dist_x_fst = 145
                    dist_x_scd = dist_x_fst + 165
                    dist_x_trd = dist_x_fst + 335
                add_annot(annot, dist_x_fst, 575, text_contour(txt_sep[0], color=color), None, 170, xanchor='center', align='center')
                add_annot(annot, dist_x_fst+5, 580, txt_sep[0], color, 170, xanchor='center', yanchor='middle')
                add_annot(annot, dist_x_scd, 545, text_contour(',' + txt_sep[1][:-1], color=color), None, 100, xanchor='center', align='center')
                add_annot(annot, dist_x_scd+5, 550, ',' + txt_sep[1][:-1], color, 100, xanchor='center', yanchor='middle')
                add_annot(annot, dist_x_trd, 575, text_contour(txt_sep[1][-1], color=color), None, 170, xanchor='center', align='center')
        ...
        if TEMPLATE != 'SBT_LOWER':
            ...
            if '/' in txt:
                txt_sep = txt.split('/')
                txt = txt_sep[0] + ' | 20' + txt_sep[1][:-1]
            add_annot(annot, DFONT_X, DFONT_Y+5, txt, '#C0DDF3', font_size+10, bg_color=None)
...
def update_graph(fig, symbol, n_months, title, subtit, dfont, bg_transparent, val_col=None):
    ...
    if show_labels:
        ...
        if titles[1] == '':
            if 'SBT_' in template_codes[template_num]:
                if TEMPLATE != 'SBT_LOWER':
                    add_annot(annot, title_x, TITLE_Y, text_contour(title, isTitle=True, color='#50B7F8'), None, 189, xanchor=xanchor, align=align)
                else:
        ...
        if 'SBT_' in template_codes[template_num]:
            if TEMPLATE == 'SBT_LOWER':
                title_x = 85
                subtit_fs += 30
                subtit_txt = 'ÍNDICE DO MÊS'
                xanchor, align = 'left', 'right'
            else:
                title_x = get_graph_center()
                xanchor, align = 'center', 'right'
...
def draw_graph(scale, symbol, n_months, title, subtit, dfont, bg_transparent, val_col=None):
    ...
    if TEMPLATE == 'SBT_LOWER' and not g001:
        add_image(fig, 'images/sbt/bg_lower.png', 0, 0,
                GWIDTH, GHEIGHT, xanchor='left', yanchor='top', layer='below')

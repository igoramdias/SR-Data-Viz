...
def grafico_degrade(input_file, output_file,delta):
    color_blue = [[70,180,255],[70,180,255]]        # Primeira cor é da linha, segunda cor do degrade inferior
    color_red = [[255,120,120],[255,120,120]]       # Primeira cor é da linha, segunda cor do degrade inferior

    imagem = Image.open(input_file)

    if imagem.mode != 'RGBA':
        raise ValueError("A imagem de entrada deve estar no modo RGBA")

    width, heigth = imagem.size
    pixels = imagem.load()

    color1 = color_blue
    color2 = color_blue if delta>=0 else color_red
    
    for x in range(width):
        per = x/float(width)
        r1 = int(color1[0][0] + (color2[0][0] - color1[0][0]) * per)
        g1 = int(color1[0][1] + (color2[0][1] - color1[0][1]) * per)
        b1 = int(color1[0][2] + (color2[0][2] - color1[0][2]) * per)
        r2 = int(color1[1][0] + (color2[1][0] - color1[1][0]) * per)
        g2 = int(color1[1][1] + (color2[1][1] - color1[1][1]) * per)
        b2 = int(color1[1][2] + (color2[1][2] - color1[1][2]) * per)
        ...
        for y in range(heigth):
            ...
            if a > 150:
                ...
                g = g2
                b = b2
                a = (int(((heigth-y) / float(heigth))*255)-10)
                if (a<0):
                    a = 0                

            pixels[x, y] = (r, g, b, a)

    imagem = imagem.resize((8000,2400),Image.LANCZOS)
...
def draw_graph_001(fig, annot, date_ini, date_end, symbol, bg_transparent, debug=None):
    ...
    try:
        ...
        if TEMPLATE not in ['SBT', 'SBT_LOWER']:
            ...
            fig_aux.add_trace(
                go.Scatter(
                    x=df.dtime[::per_intspace[period]], 
                    y=df.price[::per_intspace[period]],
                    fill='none', 
                    mode='lines', 
            ...
            fig_aux.write_image('line.png')
            grafico_degrade('line.png', 'line-gradient.png',direction)
            add_image(fig, 'line-gradient.png', 0, 150, 990, 300, xanchor='left', yanchor='top', layer='above')
            
        fig.write_image('official.png')
